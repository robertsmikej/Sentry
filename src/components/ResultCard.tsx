...
interface ResultCardProps {
  ...
  // Encounter tracking
  onEncounterUpdate?: (notes: string, experience: Experience) => void
}
...
function GlanceableBanner({
  ...
  const getBannerConfig = () => {
    ...
    return {
      ...
      title: 'KNOWN',
      subtitle:
        matchedEntry?.experience === 'bad'
          ? 'Bad experience recorded'
          : 'In your watch list'
    }
  }
  ...
  return (
    <div
      className={`${config.bgClass} ${config.textClass} -mx-4 -mt-4 mb-4 py-6 px-4`}
    >
      {/* Large centered icon and status */}
      <div className="flex flex-col items-center text-center">
        {/* Big icon */}
        <div className={`mb-2 ${config.iconClass}`}>{config.icon}</div>
        ...
        {matched && matchedEntry && (
          <div className="flex gap-4 mt-2 text-sm opacity-80">
            ...
            {matchedEntry.lastSeen && (
              <span>
                Last: {new Date(matchedEntry.lastSeen).toLocaleDateString()}
              </span>
            )}
          </div>
        )}
...
export function ResultCard({
  ...
  // Encounter notes/experience state
  const [encounterNotes, setEncounterNotes] = useState('')
  const [encounterExperience, setEncounterExperience] =
    useState<Experience>('neutral')
  ...
  return (
    <div
      className={`card w-full max-w-md shadow-xl overflow-hidden border-4 ${getBorderClass()}`}
    >
      <div className="card-body p-4">
        ...
        <div className="mb-2">
          ...
          <input
            ...
            className="input input-bordered font-mono text-4xl font-black tracking-wider text-center w-full focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2 focus:ring-offset-base-100 placeholder:text-base placeholder:font-normal"
          />
        </div>
          ...
          <div className="mt-2">
            ...
            >
              ...
              <svg
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className={`w-4 h-4 transition-transform ${showEncounterForm ? 'rotate-180' : ''}`}
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
            ...
            {showEncounterForm && (
              <div className="bg-base-100/80 rounded-lg p-3 mt-2 shadow-sm border border-base-300 space-y-3">
                ...
                <div className="form-control">
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() =>
                        setEncounterExperience(
                          encounterExperience === 'good' ? 'neutral' : 'good'
                        )
                      }
                      className={`btn flex-1 gap-2 ${
                        encounterExperience === 'good'
                          ? 'btn-success'
                          : 'btn-outline btn-sm'
                      }`}
                    >
                    ...
                    <button
                      ...
                      onClick={() =>
                        setEncounterExperience(
                          encounterExperience === 'bad' ? 'neutral' : 'bad'
                        )
                      }
                      className={`btn flex-1 gap-2 ${
                        encounterExperience === 'bad'
                          ? 'btn-error'
                          : 'btn-outline btn-sm'
                      }`}
                    >
                ...
                <div className="form-control">
                  <label className="label py-1">
                    <span className="label-text text-sm">Notes (optional)</span>
                  </label>
                  <textarea
                ...
                <button
                  onClick={() => {
                    onEncounterUpdate(encounterNotes, encounterExperience)
        ...
        <div className="card-actions flex-col gap-3">
          <button
            ...
            className={`btn btn-lg w-full min-h-[56px] ${
              saveSuccess ? 'btn-success' : 'btn-warning text-black'
            }`}
          >
            {isSaving ? (
              <>
          ...
          {/* Remove from Watchlist - only show for matched plates */}
          {matched && onRemove && (
            <button
              onClick={onRemove}
              disabled={isRemoving}
